...
public class GPSTracker extends Service implements LocationListener {
    ...
    boolean isGPSEnabled = true;// flag for network status
    boolean isNetworkEnabled = true; // flag for GPS status
    boolean canGetLocation = true;
    Location location; // location
    double latitude; // latitude
    double longitude; // longitude

    private static final long MIN_DISTANCE_CHANGE_FOR_UPDATES = 1; // 1 meters -The minimum distance to change Updates in meters
    private static final long MIN_TIME_BW_UPDATES = 1000 * 10 * 1; // 10sec -The minimum time between updates in milliseconds
    ...
    public Location getLocation() {
        try {
            ...
            if (!isGPSEnabled && !isNetworkEnabled) {
                Toast.makeText(this, "Enable your Location or Network Connection and Continue", Toast.LENGTH_SHORT).show();
            } else {
    ...
    public void showSettingsAlert(){
        ...
        alertDialog.setPositiveButton("Settings", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog,int which) {
                Intent intent = new Intent(Settings.ACTION_LOCATION_SOURCE_SETTINGS);   // On pressing Settings button
        ...
        alertDialog.setNegativeButton("Cancel", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
            dialog.cancel();   // on pressing cancel button
            }
        });
